...
TEST(VoxelGridTest, PopulateFromObstacles) {
    ...
    grid.populate_from_obstacles(obstacles, 1.0);
...
TEST(VoxelGridTest, BenchmarkHashSetVsRingBuffer) {
    ...
    auto ring_us = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3).count();
